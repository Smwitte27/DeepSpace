package frc.team2606.robot.planners;
...
import frc.team2606.lib.physics.DCMotorTransmission;
import frc.team2606.lib.physics.DifferentialDrive;
import frc.team2606.lib.trajectory.*;
import frc.team2606.lib.trajectory.timing.DifferentialDriveDynamicsConstraint;
import frc.team2606.lib.trajectory.timing.TimedState;
import frc.team2606.lib.trajectory.timing.TimingConstraint;
import frc.team2606.lib.trajectory.timing.TimingUtil;
import frc.team2606.lib.util.CSVWritable;
import frc.team2606.lib.util.Units;
import frc.team2606.lib.util.Util;
import frc.team2606.robot.Constants;
...
public class DriveMotionPlanner implements CSVWritable {
    ...
    public DriveMotionPlanner() {
        final DCMotorTransmission transmission = new DCMotorTransmission(
                1.0 / Constants.driveKv,
                Units.inches_to_meters(Constants.driveWheelRadiusInches)
                        * Units.inches_to_meters(Constants
                        .driveWheelRadiusInches) * Constants.robotLinearInertia / (2.0 * Constants.driveKa),
                Constants.driveVIntercept);
        mModel = new DifferentialDrive(
                Constants.robotLinearInertia,
                Constants.robotAngularInertia,
                Constants.robotAngularDrag,
                Units.inches_to_meters(Constants.driveWheelDiameterInches / 2.0),
                Units.inches_to_meters(Constants.driveWheelWidthInches / 2.0 * Constants.trackScrubFactor),
    ...
    protected Output updatePurePursuit(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
        double lookahead_time = Constants.pathLookaheadTime;
        ...
        while (actual_lookahead_distance < Constants.pathMinLookaheadDistance
                &&
        ...
        if (actual_lookahead_distance < Constants.pathMinLookaheadDistance) {
            lookahead_state = new TimedState<>(new Pose2dWithCurvature(lookahead_state.state()
                    .getPose().transformBy(Pose2d.fromTranslation(new Translation2d(
                            (mIsReversed ? -1.0 : 1.0) * (Constants.pathMinLookaheadDistance -
        ...
        DifferentialDrive.ChassisState adjusted_velocity = new DifferentialDrive.ChassisState();
        // Feedback on longitudinal error (distance).
        adjusted_velocity.linear = dynamics.chassis_velocity.linear + Constants.pathKX * Units.inches_to_meters
                (mError.getTranslation().x());
